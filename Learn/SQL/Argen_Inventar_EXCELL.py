...
book = openpyxl.open(r"\\srv-file\IT\Dokumentation\Dokumentationen_Daniel\MS_SQL\test.xlsx")
sheet = book.active
...
class App(customtkinter.CTk):
    ...
    def first_frame_writing_data(self):
        ...
        if len(self.first_frame_artikel_entry.get()) > 0:
            ...
            sheet.cell(self.max_row, column=1).value = None
            sheet.cell(self.max_row, column=2).value = None
            sheet.cell(self.max_row, column=3).value = self.first_frame_artikel_entry.get().capitalize().strip()
            sheet.cell(self.max_row, column=4).value = self.first_frame_hersteller_entry.get().capitalize().capitalize().strip()
            sheet.cell(self.max_row, column=5).value = self.first_frame_model_entry.get().strip()
            sheet.cell(self.max_row, column=6).value = self.first_frame_sn_entry.get().strip()
            sheet.cell(self.max_row, column=7).value = self.first_frame_datum_entry.get().strip()
            sheet.cell(self.max_row, column=8).value = self.first_frame_bemerkung_entry.get('0.0', 'end').strip()

            book.save(r"\\srv-file\IT\Dokumentation\Dokumentationen_Daniel\MS_SQL\test.xlsx")
    ...
    def second_frame_lager_tabelle(self):
        ...
        for row in sheet.iter_rows():
            ...
            records_invertar_second_frame.append(sub_list)

        second_liste_finish = []
        for rows in records_invertar_second_frame:
            if rows[0] == None:
                second_liste_finish.append(rows[2:])

        for values in second_liste_finish:
            if values[5] == None:
                values[5] = ""
        ...
        count = 0
        for record in second_liste_finish:
            if count % 2 != 0:
    ...
    def uebergabe_function(self):
        ...
        #    for cell in row:
        #        sub_list.append(cell.value)
        #    vorname_list.append(sub_list)
        #print("VORNAME LIST 1", vorname_list)
        #name_liste_finish = []
        #for rows in vorname_list:
        #    if rows[0] != None:
        #        name_liste_finish.append(rows[0])

        self.name_liste_finish = [rows[:2] for rows in [[cell.value for cell in row] for row in sheet.iter_rows()] if
                             rows[0] is not None]
        self.vorname_liste = sorted(set([rows[0] for rows in self.name_liste_finish]))
    ...
    def vorwahl_nachname(self, name):
        ...
        nachname_list = sorted(set([nachname[1] for nachname in self.name_liste_finish if name in nachname]))
